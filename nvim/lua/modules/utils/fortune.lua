...
local function draw_box(lines)
    ...
    for _, l in ipairs(lines) do
        ...
        if l:sub(1, string.len('——')) == '——' then
            -- 人名右对齐
            formatted = string.format('%s %s%s %s', box_chars.sides, string.rep(' ', offset), l, box_chars.sides)
        else
            -- 语录左对齐
            formatted = string.format('%s %s%s %s', box_chars.sides, l, string.rep(' ', offset), box_chars.sides)
        end
